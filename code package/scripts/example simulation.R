...
r4<-inverted_test_gridsearch_cis(grid=seq(-100+r3$coefficients["x"],100+r3$coefficients["x"],length.out=15),
                                 y=y,x=x,z=z,level=ci_level,mode=mode,plot=T,testfunction = ri_iv_test_p)

print(r4$ci)
r5<-inverted_test_gridsearch_cis(grid=seq(-100+r3$coefficients["x"],100+r3$coefficients["x"],length.out=15),
...
for (i in 1:5000) {
  ...
  r4<-inverted_test_gridsearch_cis(grid=seq(-20+r3$coefficients["x"],20+r3$coefficients["x"],length.out=7),
                       y=y,x=x,z=z,level=ci_level,mode=mode,testfunction=ri_iv_test_p,r=999)
  ...
  reject_the_truth_wbs<-wild_iv(b_hypothezised=treatment_effect,y=y,x=x,z=z,r=999)
