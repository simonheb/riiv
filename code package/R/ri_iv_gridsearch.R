...
ri_iv_gridsearch<-function(
  y,x,z,               #data
  grid,                #prespecified search grid optional
  allowInf=F,          #return Inf for upper and lower CI bound if the grid-ranges cannot be rejected
  level=0.9,           #ci-level
  plot=FALSE,          #plot results?
  refinementsteps=1,   #how many recursive refinement steps?
  refinementsresolution=11,            #steps to divide this into
  ...
  for (b in grid) {
    #copute p-value
    pval<-ri_iv_test_p(b=b,y=y,x=x,z=z,r=r,seed=seed,ranktest=wilcox) #this function is defined in src/ri_iv_test_cpp.cpp
  ...
  if (refinementsteps) {
    ...
    if (!is.infinite(gridres[1,1]))
      refined_grid_lower_bound<-seq(gridres[min(unable_to_reject)-1,1],gridres[min(unable_to_reject),1],length.out=ceiling(refinementsresolution/2))
    #if upper bound is finite:
    if (!is.infinite(gridres[nrow(gridres),1]))
      refined_grid_upper_bound<-seq(gridres[max(unable_to_reject),1],gridres[max(unable_to_reject)+1,1],length.out=ceiling(refinementsresolution/2))
    ...
    if (length(refined_grid)>0) {
      #call function
      return(ri_iv_gridsearch(grid=refined_grid,y=y,x=x,z=z,level=level,plot=plot,refinementsteps=refinementsteps-1,allowInf=allowInf,r=r,seed=seed,refinementsresolution=refinementsresolution,gridres=gridres,verbose=verbose))
    }
  }
